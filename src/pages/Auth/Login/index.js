...
const Login = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      const { data: { token, user } } = await axios.post(login, { ...credentials })
  ...
  return (
    <div className="container d-flex justify-content-center align-items-enter flex-column w-50" style={{ height: "100vh", width: "100%" }}>
      <h3>Login Form</h3>
      <p className="text-muted fs-6">New User? <Link to="/auth/signup" className="text-decoration-underline text-black">Register Now</Link></p>
